...
class ArucoNode(rclpy.node.Node):
    ...
    def __init__(self):
        ...
        # CompressedImage subscriber
        self.create_subscription(
            CompressedImage,
            self.params_dict['image_topic'], self.compressed_image_callback, qos_profile_sensor_data)
    ...
    def compressed_image_callback(self, img_msg):
        self.get_logger().info("HEREEE")
        if self.info_msg is None:
            self.get_logger().warn("No camera info has been received!")
            return

        try:
            cv_image = self.brige.compressed_imgmsg_to_cv2(img_msg, "passthrough")
